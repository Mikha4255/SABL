...
(() => {
    ...
    function openModal2(title, text, imgSrc) {
        if (!modal2 || !modal2Title || !modal2Text || !modal2Img) return;
        modal2Title.textContent = title;
        modal2Text.textContent = text;
        modal2Img.src = imgSrc;
        modal2.classList.add('active');
        setTimeout(() => modal2.classList.add('visible'), 200);
    }
    
    function closeModal2() {
        if (!modal2) return;
        modal2.classList.remove('visible');
        setTimeout(() => modal2.classList.remove('active'), 300);
    }
    
    modalClose2?.addEventListener('click', closeModal2);
    modalOverlay2?.addEventListener('click', closeModal2);
    
    document.querySelectorAll('#year2026 .most_nom .nomcard, #year2026 .extra_nom .nomcard').forEach(card => {
        card.addEventListener('click', (e) => {
            if (e.target.closest('.choose-nominee-btn')) return;
